...
class Solution {
    public:
        ...
        int jump(vector<int>& nums) {
            ...
            printf("b/f while loop\n\n");

            while (!(jumps_pos.empty())) {

                printf("in while loop\n\n");

                // print mm for sanity check
                for (check_iter = jumps_pos.begin(); check_iter != jumps_pos.end(); check_iter++) {
                    printf("jumps: %d | pos: %d\n", check_iter->first, check_iter->second);
                }
                printf("\n");
...
printf("index: %d, jumps_to_here: %d\n", index, jumps_to_here);
